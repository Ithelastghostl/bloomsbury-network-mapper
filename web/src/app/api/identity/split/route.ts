...
// POST /api/identity/split — store not-same-person decision (§15.5, §20.4, §24)
export const POST = withIdempotency(async (request: Request) => {
  try {
    ...
    if (entity_a_mention_id === entity_b_mention_id) {
      return apiError('bad_request', 'Cannot split an entity from itself');
    }
    ...
    if (error) {
      return serverError('Failed to create split decision', error.message);
    }
